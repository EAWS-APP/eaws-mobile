...
class _SOSScreenState extends State<SOSScreen> with TickerProviderStateMixin {
  ...
  Future<void> _activateSOSBroadcast() async {
    ...
    if (connectivityResult == [ConnectivityResult.none] || connectivityResult.contains(ConnectivityResult.none)) {
    ...
    try {
      ...
      String resolvedAddress = '${position.latitude.toStringAsFixed(5)}, ${position.longitude.toStringAsFixed(5)}';
      try {
      ...
      setState(() {
        ...
        _activeIncidentId = incident['id']?.toString() ?? incident['incident_id']?.toString();
      });
  ...
  Future<void> _triggerOfflineSMSFallback() async {
    ...
    final String message = 'EAWS EMERGENCY SOS! CitizenEbenezar triggered alert. Location: Lat ${lat.toStringAsFixed(5)}, Lng ${lng.toStringAsFixed(5)} (https://maps.google.com/?q=$lat,$lng)';
  ...
  void _startBackgroundTracking() {
    _backgroundTrackingTimer?.cancel();
    _backgroundTrackingTimer = Timer.periodic(const Duration(seconds: 15), (timer) async {
      final incidentId = _activeIncidentId;
      ...
      try {
        ...
        await EawsApiClient.instance.patch('/incidents/$incidentId', body: {
          'latitude': position.latitude,
          'longitude': position.longitude,
        });

        print('EAWS Telemetry background sync successful: ${position.latitude}, ${position.longitude}');
      } catch (e) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildTelemetryRow(
                      ...
                      'GPS Accuracy',
                      _gpsAccuracy != null
                          ? '±${_gpsAccuracy!.toStringAsFixed(1)} meters'
                          : '±3 meters (High Accuracy)',
                    ),
                    const Divider(height: 20),
                    _buildTelemetryRow(
